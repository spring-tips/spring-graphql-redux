...
/**
 * Be sure to start both the {@code queries} example and the {@code rsocket} example
 * before running this
 */
	...
	ApplicationRunner applicationRunner(HttpGraphQlClient httpGraphQlClient,
		...
		return args -> {
			var httpDocument = """
					...
					""";
			httpGraphQlClient.document(httpDocument).retrieve("customerById").toEntity(Customer.class)
			...
			var rsocketDocument = """
					...
					""";
			rSocketGraphQlClient.document(rsocketDocument).retrieveSubscription("greetings").toEntity(Greeting.class)
					.subscribe(System.out::println);
